...
func defaultConfig() config {
	return config{
		Logger:   slog.Default(),
		Endpoint: "/events/callback",
		Verifier: httpinteraction.DefaultVerifier{},
	}
}
...
type config struct {
	Logger          *slog.Logger
	Endpoint        string
	Verifier        httpinteraction.KeyVerifier
...
// WithEndpoint sets the endpoint discord will send webhook events to.
...
func WithVerifier(verifier httpinteraction.KeyVerifier) ConfigOpt {
	return func(config *config) {
