...
func (c *connImpl) handleMessage(_ Gateway, op Opcode, sequenceNumber int, data GatewayMessageData) {
	switch d := data.(type) {
	...
	if c.config.EventHandlerFunc != nil {
		c.config.EventHandlerFunc(c.gateway, op, sequenceNumber, data)
	}
}
