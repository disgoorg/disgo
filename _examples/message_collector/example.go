	...
	if event.Message.Content == "start" {
		go func() {
			...
			for {
				select {
				case <-ctx.Done():
					_, _ = event.Client().Rest().Channel().CreateMessage(event.ChannelID, discord.NewMessageCreateBuilder().SetContent("cancelled").Build())
					return
				...
				case messageEvent := <-ch:
					...
					if i == 3 {
						cls()
						_, _ = event.Client().Rest().Channel().CreateMessage(messageEvent.ChannelID, discord.NewMessageCreateBuilder().SetContent(str).Build())
					}
					i++
				}
			}
